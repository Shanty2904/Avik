...
// ─────────────────────────────────────────────────────────
// useSlideScroll — rewritten for performance
//
// Key design decisions:
// • currentIndexRef tracks position for scroll logic (no re-renders)
// • currentIndex state is only updated AFTER animation completes
//   so React only re-renders once per slide, not every RAF frame
// • isAnimating, accumulated are refs — never cause re-renders
// • Event listeners attach ONCE on mount, never re-attach
// • Trackpad: accumulates deltaY, fires only past threshold
...
function useSlideScroll(containerRef, totalSections) {
  ...
  const slideTo = useCallback((index) => {
    ...
    rafId.current = requestAnimationFrame(step)
  }, [containerRef, totalSections]) // stable — totalSections never changes
  ...
  useEffect(() => {
    ...
    function onWheel(e) {
      ...
      // Trackpad sends many small events — accumulate them
      // Mouse wheel sends large discrete events (±120) — fire immediately
      const isTrackpad = Math.abs(delta) < 50

      if (isTrackpad) {
        // Reset accumulator if there was a pause (new gesture)
        if (now - lastWheelTime.current > 150) accumulated.current = 0
        ...
        // Mouse wheel — fire directly
        if (!isAnimating.current) {
          slideTo(currentIndexRef.current + (delta > 0 ? 1 : -1))
        }
      }
    }
...
// ─────────────────────────────────────────────────────────
function SectionFrame({ title, bg, children, id, noDivider = false, titleFrameIndex, contentFrameIndex, currentIndex }) {
        ...
        <div className="film-track flex w-max">
          {items.map((item, i) => (
            <Link key={i} to={item.href} className="relative mr-4 md:mr-8 flex-shrink-0 group">
              <img src={item.src} alt={item.alt} className="h-[180px] md:h-[350px] w-auto object-contain block" loading='lazy' />
