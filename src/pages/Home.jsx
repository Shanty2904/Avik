...
// ─────────────────────────────────────────────────────────
// Easing function — ease in-out cubic, feels cinematic
// ─────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────
// useSlideScroll
// Intercepts wheel, touch, and keyboard events on the snap
// container and animates scrollTop to the nearest section
// boundary using requestAnimationFrame + custom easing.
...
function useSlideScroll(containerRef, totalSections) {
  ...
  const slideTo = useCallback((index) => {
    ...
    if (clampedIndex === currentIndex && container.scrollTop === clampedIndex * container.clientHeight) return
...
// ─────────────────────────────────────────────────────────
// SectionFrame
// Two sections per topic:
//   Stop 1 (title frame) — title centered, full opacity
//   Stop 2 (content frame) — title centered, 40% opacity,
//                             content fades up below it
//
// currentIndex and frameIndex are passed from Home so
// SectionFrame knows if it's the active content frame.
// ─────────────────────────────────────────────────────────
function SectionFrame({ title, bg, children, id, noDivider = false, titleFrameIndex, contentFrameIndex, currentIndex }) {
  ...
  return (
    <>
      ...
      {/* ── STOP 2: Content frame — title fades to 40%, content slides up ── */}
      <section
        id={id}
        className={`snap-section ${bg} relative flex flex-col overflow-hidden`}
      >
        {/* Title — same position, just dimmed */}
        ...
        {/* Content — fades and slides up when active */}
        <div
...
// ─────────────────────────────────────────────────────────
// Section components — receive currentIndex from Home
// ─────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────
// HOME ROOT
// Total sections: 1 hero + 5 × (title + content) = 11
